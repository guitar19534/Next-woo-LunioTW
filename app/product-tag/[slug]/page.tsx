...
function parsePriceHtml(html: string): string {
  const nums = html.match(/[\d,]+/g)?.filter((n) => n.replace(/,/g, "").length >= 3) ?? [];
    ...
    <div style={{
      ...
      <div style={{ padding: "18px 18px 0", flexGrow: 1, display: "flex", flexDirection: "column" }}>
        <Link href={`/product/${p.slug}`} style={{ textDecoration: "none" }}>
          <h2 style={{ fontSize: "clamp(14px,1.3vw,16px)", fontWeight: 700, color: NAVY, marginBottom: 8, lineHeight: 1.4 }}
        ...
        {features.length > 0 ? (
          ...
          <div className="wc-desc" style={{ flexGrow: 1, fontSize: "clamp(12px,0.95vw,13.5px)", color: "#555", lineHeight: 1.8, marginBottom: 16 }}
    ...
    <main style={{ fontFamily: "'MiSansTC','Noto Sans TC',sans-serif", color: "#212020", background: "#fff" }}>
      ...
      <section style={{ background: "#fff", padding: "clamp(40px,5vw,64px) 0 clamp(32px,4vw,48px)" }}>
        <div style={W}>
          ...
          {tag.description && (
            <div
              style={{ fontSize: "clamp(13px,1.1vw,16px)", color: "#444", lineHeight: 1.9, maxWidth: 960 }}
        ...
        <div style={W}>
          {products.length === 0 ? (
            ...
            <div className={`grid gap-5 ${colClass}`}>
              {products.map((p) => <WcProductCard key={p.id} p={p} />)}
            </div>
          )}
        </div>
      </section>
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-x-4 gap-y-8">
            {SERVICES.map((s) => (
              <div key={s.label} style={{ textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                ...
                <p style={{ fontSize: "clamp(11px,.9vw,13px)", fontWeight: 600, color: NAVY, lineHeight: 1.4 }}>
                  {s.label}
                </p>
              </div>
            ))}
