        ...
        >
          {FEATURES.map((f, i) => (
            <div
              ...
              style={{
                width: "clamp(260px, calc((100% - 48px) / 3.5), 400px)",
                minWidth: "260px",
...
export function Gen4Pricing() {
  const [activeTab, setActiveTab] = useState(1); // default: 標準雙人 (5尺)
    ...
    <div className="bg-white pt-4 pb-16 md:pt-6 md:pb-24">
      <div className="max-w-[1140px] mx-auto px-5 sm:px-8">
        ...
        <div className="text-center mb-8 md:mb-14">
          <h2 className="font-bold" style={{ fontFamily: "'Chiron Goround TC', sans-serif", fontSize: "clamp(22px,3vw,30px)", fontWeight: 700, color: "#17569E" }}>
            適合全家人的 Lunio
          </h2>
        </div>
        ...
        <div className="lg:hidden">
          <style>{`.size-track::-webkit-scrollbar{display:none}`}</style>
          <div className="flex items-center gap-2">

            {/* Left arrow */}
            <button
              type="button"
              onClick={() => { const el = document.querySelector('.size-track') as HTMLElement; el?.scrollBy({ left: -300, behavior: 'smooth' }); }}
              aria-label="上一個"
              className="flex-shrink-0 flex items-center justify-center w-9 h-9 rounded-full border transition-colors"
              style={{ borderColor: "#17569E", color: "#17569E" }}
            >
            ...
            {/* Track */}
            <div
              className="size-track flex gap-4 overflow-x-auto pb-2 flex-1"
              style={{ scrollSnapType: "x mandatory", scrollbarWidth: "none" }}
            >
              {SIZES.map((s, i) => (
                <div
                  key={i}
                  className="flex-shrink-0 rounded-2xl overflow-hidden border border-gray-100"
                  style={{ width: "75vw", maxWidth: "300px", scrollSnapAlign: "center" }}
                >
                  <div className="relative w-full bg-white" style={{ aspectRatio: "4 / 3" }}>
                    <Image src={s.image} alt={`${s.label} ${s.cm}`} fill className="object-contain" sizes="75vw" />
                  </div>
                  <div className="p-4 text-center">
                    <p className="font-bold mb-1" style={{ fontSize: "15px", color: "#17284b", fontFamily: "'Chiron Goround TC', sans-serif" }}>{s.label}</p>
                    <p className="mb-3" style={{ fontSize: "12px", color: "#888", letterSpacing: "0.05em" }}>{s.tw}</p>
                    <p className="mb-4" style={{ fontSize: "13px", color: "#5F6062", lineHeight: 1.6 }}>{s.desc}</p>
        ...
        <div className="hidden lg:block">
          ...
          {/* Product image */}
          <div className="relative w-full overflow-hidden rounded-2xl md:rounded-3xl mb-10 bg-white border border-gray-100" style={{ aspectRatio: "16 / 7" }}>
            <AnimatePresence initial={false} custom={direction} mode="wait">
              ...
              >
                <Image src={current.image} alt={`Lunio Gen4 ${current.label}`} fill className="object-contain" sizes="1140px" />
              </motion.div>
            </AnimatePresence>
          </div>

          {/* Tab bar */}
          <div className="relative flex gap-6 border-b border-gray-200 mb-10">
            {SIZES.map((s, i) => (
              <button key={i} type="button" onClick={() => selectTab(i)}
                className="relative flex-1 py-4 px-5 text-center transition-colors duration-200"
                style={{ fontFamily: "'Chiron Goround TC', sans-serif", fontSize: "13px", letterSpacing: "0.10em", fontWeight: i === activeTab ? 700 : 400, color: i === activeTab ? "#17284b" : "#888888", whiteSpace: "nowrap" }}
              >
                {s.label}
                <span className="block text-[11px] mt-0.5" style={{ color: i === activeTab ? "#17569E" : "#ccc" }}>{s.tw}</span>
                {i === activeTab && (
                  <motion.span layoutId="tab-indicator" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#17569E] rounded-full" transition={{ type: "spring", stiffness: 400, damping: 35 }} />
                )}
              </button>
            ))}
          </div>
          ...
          <div className="relative overflow-hidden" style={{ minHeight: "120px" }}>
            <AnimatePresence initial={false} custom={direction} mode="wait">
              <motion.div key={activeTab} custom={direction}
                variants={prefersReduced ? undefined : {
                  enter: () => ({ y: "12px", opacity: 0 }),
                  center: { y: 0, opacity: 1 },
                  exit: () => ({ y: "-12px", opacity: 0 }),
                }}
                initial="enter" animate="center" exit="exit"
                transition={{ duration: 0.25, ease: "easeOut" }}
              >
                <p className="text-center mb-8" style={{ fontSize: "16px", fontFamily: "'Chiron Goround TC', sans-serif", letterSpacing: "0.10em", color: "#212020", lineHeight: 1.7 }}>
                  {current.desc}
                </p>
                <div className="flex justify-center">
                  <div className="flex items-center rounded-full border-2 overflow-hidden" style={{ borderColor: "#17569E" }}>
