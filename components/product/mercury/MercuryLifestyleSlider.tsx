...
const SLIDES = [
  { img: "/mercury/PC_1.webp", imgMobile: "/mercury/Mobile_1.webp" },
  { img: "/mercury/PC_2.webp", imgMobile: "/mercury/Mobile_2.webp" },
  { img: "/mercury/PC_3.webp", imgMobile: "/mercury/Mobile_3.webp" },
];

export function MercuryLifestyleSlider() {
  ...
  return (
    <section className="bg-white py-10 md:py-14 overflow-hidden">
      ...
      <div className="max-w-[1280px] w-[90%] mx-auto text-center mb-6 md:mb-8">
        <h2 className="font-bold mb-2" style={{ fontSize: "clamp(20px, 2.5vw, 32px)", color: "#17284b" }}>
          更放鬆，更好睡
        </h2>
        <p className="hidden md:block" style={{ fontSize: "clamp(13px, 1.2vw, 15px)", color: "#6b7280" }}>
          機能性材質與個性化高度，提供肩頸最佳支撐，讓你更放鬆容易入睡
        </p>
      </div>
      ...
      {/* Mobile slider — 2300×2850 portrait */}
      <div className="md:hidden w-[85%] mx-auto">
        <div className="relative w-full overflow-hidden rounded-2xl" style={{ aspectRatio: "2300 / 2850" }}>
          {SLIDES.map((s, i) => (
            <div key={i} className="absolute inset-0 transition-opacity duration-700"
              style={{ opacity: i === current ? 1 : 0 }}>
